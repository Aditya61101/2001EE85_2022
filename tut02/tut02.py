...
def octant_identification(mod):
    ...
    for i in range(2,row_count+1):
        ...
        octant_sign=checkOct(sub_u_avg,sub_v_avg,sub_w_avg)
        if octant_sign==1:
...
def setOverAllTransitionCount():
    ...
    start = 0
    last = sheet.cell(row=2,column=11).value
    while(start<total_count-1):
        curr_value = sheet.cell(row=start+3, column=11).value
        transition_count[str(last) + str(curr_value)]+=1
        last = curr_value
        start += 1
